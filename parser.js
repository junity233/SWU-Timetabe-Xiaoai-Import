  ...
  $('#kbgrid_table_0 td').each((i, td) => {
    if ($(td).hasClass('td_wrap') && $(td).text().trim() !== '') {
      ...
      $(td).find('.timetable_con.text-left').each((i, course) => {
        const name =
            $(course).find('.title font').text().replace(regexName, '').trim();
        ...
        if (infoStr && infoStr.match(/\((\d+-\d+节)\)/) &&
            infoStr.split('节)')[1]) {
  ...
  $('#kblist_table tbody').each((day, tbody) => {
    if (day > 0 && day < 8) {
      ...
      $(tbody).find('tr:not(:first-child)').each((trIndex, tr) => {
        ...
        const weeks =
            parserWeeks($(font[0]).text().replace(regexWeekNum, '').trim());
        const position =
            $(font[1]).text().replace(regexPosition, '').trim().split(' ')[1];
...
function parserInfo(str) {
  const sections =
      parserSections(str.match(/\((\d+-\d+节)\)/)[1].replace(/节/g, ''));
